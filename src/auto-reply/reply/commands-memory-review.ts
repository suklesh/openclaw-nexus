...
export async function handleMemoryReviewCommands(
  ...
  if (!body.startsWith("/memory-review") && !body.startsWith("/memoryreview") && !body.startsWith("/remember")) {
    return null;
  }
  ...
  if (!indexOneBased || indexOneBased < 1) {
    return {
      reply: { text: "Usage: /memory-review keep|edit|discard|defer <n> ... (run /memory-review to see n)" },
      shouldContinue: false,
    };
  }
