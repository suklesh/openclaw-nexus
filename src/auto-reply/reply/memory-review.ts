...
import { loadSessionStore, updateSessionStore, updateSessionStoreEntry } from "../../config/sessions.js";
...
function resolveDailyMemoryPath(cfg: OpenClawConfig, workspaceDir: string, now = new Date()): string {
...
export async function applyMemoryReviewAction(params: {
  ...
  if (params.action === "keep") {
    await appendToDailyMemory({ cfg: params.cfg, workspaceDir: params.workspaceDir, text: picked.text });
